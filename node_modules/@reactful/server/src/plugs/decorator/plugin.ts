...
import { parenthelessDecorators, removeAllCommentsLines } from './fixings'
import regexes, { IS_ANONYMOUS } from './regex'

interface Query { name: string, call: string, expr: string }
...
export default function(args: Args) {
   ...
   if (!functions.length || !decorators.length) return presentCode
   ...
   for (const fn of functions) {
      if (decorators.every(d => d.call != fn.name)) continue
...
export function extractDecorators(args: Args): Query[] { 
   let regex = undefined as any, results = [] as Query[]
