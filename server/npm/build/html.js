...
export async function HTMLizer(request) {
    ...
    const [outerHtml, innerHtml] = html.match(HTML_REGEX) || [];
    const [outerHead, innerHead] = html.match(HEAD_REGEX) || [];
    const [outerBody, innerBody] = html.match(BODY_REGEX) || [];
    if (outerHtml)
        html = html.replace(outerHtml, innerHtml);
...
async function transformFromJSX(name, html) {
    const imports = [];
