...
export default async function(jsx: RFC|null, route: string, inner: string, outer?: string) {
...
async function injectInHTML(route: string, inner: string, outer: string) {   
   const INIT_REGEX = /^[\s\S]*<html.*?>/gm  

   const jdom = new JSDOM(outer).window.document as Document
   const html = jdom.documentElement.innerHTML
   const node = jdom.querySelector(settings.queryId)

   if (!node) return throws<string>(NOT_FOUND_ROOT)
...
function extractingFromHTML(dom: any, field: string): any|undefined {
...
function extractingMetaTagsFromHTML(html: string): MetaTags|undefined {
   ...
   return jsxMetaData as MetaTags
}

function createOgEntries(key: string, obj: any): [string, any] {
