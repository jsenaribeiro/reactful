...
test('@auth: authorized', function () {
   ...
   const result = auth()(import.meta, fComponent) as Function
...
test('@auth: unauthorized role', function () {
   ...
   const result = auth({ role: 'user' })(import.meta, fComponent) as Function
   const element = result() as any
   const message = element.props.children[1]
      .props.children[0].props.children
...
test('@auth: authorized role', function () {
   ...
   const result = auth({ role: 'admin' })(import.meta, fComponent) as Function
   const element = result() as any
